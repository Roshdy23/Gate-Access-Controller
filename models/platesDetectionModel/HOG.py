...
class HOG:
    ...
    def compute_hog_features(self, image: np.ndarray) -> np.ndarray:
        ...
        num_cells_y = grad_magnitude.shape[0] // 8
        num_cells_x = grad_magnitude.shape[1] // 8
        cells_histogram = np.zeros((num_cells_y, num_cells_x, len(hist_bins)))

        for r in range(0, grad_magnitude.shape[0] - 7, 8):
            for c in range(0, grad_magnitude.shape[1] - 7, 8):
                cell_direction = grad_direction[r:r+8, c:c+8]
                cell_magnitude = grad_magnitude[r:r+8, c:c+8]
                cells_histogram[r // 8, c // 8] = self.calculate_histogram_per_cell(cell_direction, cell_magnitude, hist_bins)
